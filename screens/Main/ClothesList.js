...
function clothesList(props) {
    //list of clothess = state
    const [clothess, setclothess] = useState([        
        {
            name: 'Paella Valenciana, with rabbit and chicken; and seaclothes paella',
            url: 'https://www.bluristorante.com/wp-content/uploads/2019/03/9-Traditional-Italian-clothes-Dishes-You-Will-Love-1080x700.jpg',
            status: 'Opening soon',
            price: 5223.56,
    ...
    const [categories, setCategories] = useState([
        {
            name: 'BBQ',
            url: 'https://images.clothesy.vn/BlogsContents/46444498_1785582584898023_6834569445101273088_n(1).jpg'
        },
        {
            name: 'Breakfast',
    ...
    const filteredclothess = () => clothess.filter(eachclothes => eachclothes.name.toLowerCase()
    ...
    return <SafeAreaView style={{flex: 1, backgroundColor: 'white'}}>
        <View>
            ...
            {/* <ScrollView>
                {clothess.map(eachclothes => <clothesItem 
                    clothes = {eachclothes} key={eachclothes.name}/>)}                
            </ScrollView> */}
            </View>                     
            {filteredclothess().length > 0 ? <FlatList 
                data={filteredclothess()}
                renderItem={({item}) => <clothesItem 
                    onPress={()=> {                        
                        alert(`You press item's name: ${item.name}`)
                    }}
                    clothes = {item} key={item.name}/>}
                keyExtractor={eachclothes => eachclothes.name}
            /> : <View style={{
                    flex: 1, 
                    justifyContent: 'center', 
                    alignItems: 'center',                    
